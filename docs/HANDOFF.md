...
- Parameter rows: **name left-aligned**, **value right-aligned** (text or checkbox)  
- **WIFI** / **OTA**: binary rows use trailing checkboxes (4 px bird-like ✓: green when unfocused, black when focused or editing; slightly larger than the box); no ON/OFF text  
...
| WIFI | checkbox | toggle | from `network.wifiEnabled` |
| OTA (ArduinoOTA on STA) | checkbox | toggle | from `network.enableOtaOnNetwork` |
